...
import { store } from "../redux/store";
 ...
 test("the login button is rendered", async () => {
     ...
     const {getByRole} = render( <BrowserRouter><Provider store={store}> <Login onExit={()=>null} onRegisterClick={()=>null}/></Provider></BrowserRouter> );
 ...
 test("the email input is rendered", async () => {
     ...
     const {getByTitle} = render( <BrowserRouter><Provider store={store}> <Login onExit={()=>null} onRegisterClick={()=>null}></Login></Provider></BrowserRouter> );
     const emailInput = getByTitle("email");
     expect(emailInput).toBeInTheDocument();

 });

test("the email title is rendered", async () => {

    const {getByText} = render( <BrowserRouter><Provider store={store}> <Login onExit={()=>null} onRegisterClick={()=>null}></Login></Provider></BrowserRouter> );
...
test("the password title is rendered", async () => {
